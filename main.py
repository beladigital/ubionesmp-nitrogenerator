...
""") # Print the title card
        ...
        self.slowType("Nitro code checker & generator by UbiOneSMP nitro miner team.", .02) # Print who developed the code
        time.sleep(1) # Wait a little more
        self.slowType("\nMennyi kódot szeretnél generálni? ", .02, newLine = False) # Print the first question

        num = int(input('')) # Ask the user for the amount of codes

        # Get the webhook url, if the user does not wish to use a webhook the message will be an empty string
        self.slowType("\nSzeretnél használni Discord Webhookot a nitro kódok kiírásához? \nAz UbiOneSMP miner team Discord Webhookja a következő: \nhttps://discord.com/api/webhooks/839823563745787946/pGSUy5fMxrm8et-bcJk1HRKSLbzb4ARXz0RKwRx-grTNV-Xxv4XBOPy7tTxG6Unsb2T3 \nHa nem szeretnél használni, csak üss egy Entert. \nDiscord Webhook: ", .02, newLine = False)
        ...
        for i in range(num): # Loop over the amount of codes to check
            ...
            if os.name == "nt": # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(f"UbiOneSMP nitro miner - {len(valid)} Valid | {invalid} Invalid") # Change the title
                print("")
            else: # If it is a unix system
                print(f'\33]0;UbiOneSMP nitro miner - {len(valid)} Valid | {invalid} Invalid\a', end='', flush=True) # Change the title

        print(f"""
Generátor eredmények:
 Valid: {len(valid)}
 Invalid: {invalid}
 Valid Codes: {', '.join(valid )}""") # Give a report of the results of the check

        input("\nVége! Nyomd le 5x az Entert, hogy kilépj a programból.") # Tell the user the program finished
